...
export function buildSanitizedDiagnosticsReport(
  ...
  const lines = [
    ...
    `Audio routine focus suppression: ${appState.settings.audio.onlyWhenUnfocused}`,
    `Audio completion quiet period: ${appState.settings.audio.completionSilenceMs}ms`,
    `Audio minimum active duration: ${appState.settings.audio.minimumActivityMs}ms`,
